---
title: "withTimeout"
description: "Sets a time limit on an asynchronous function, throwing a TimeoutError if it doesn't complete within the specified time."
---

import { Badge, Field } from '@ciderpress/ui/theme'

# withTimeout <Badge color="#c85a3e">es-toolkit</Badge>
...
<Field name="run" type="() =&gt; Promise&lt;T&gt;" required>
...
<Field name="options" type="WithTimeoutOptions">
Timeout options.
...
Re-exported verbatim from [`es-toolkit`](https://es-toolkit.dev/reference/promise/withTimeout.html). Implementation, edge cases, and performance behavior are owned upstream. This page mirrors the documentation at the pinned version; the linked source is authoritative.
