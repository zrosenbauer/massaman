---
title: "filter"
description: "Creates a function that keeps only the elements that pass a test, equivalent to Array.prototype.filter."
---

import { Badge, Field } from '@ciderpress/ui/theme'

# filter (Functional Programming) <Badge color="#c85a3e">es-toolkit</Badge>
...
Re-exported verbatim from [`es-toolkit`](https://es-toolkit.dev/fp/reference/filter.html). Implementation, edge cases, and performance behavior are owned upstream. This page mirrors the documentation at the pinned version; the linked source is authoritative.
