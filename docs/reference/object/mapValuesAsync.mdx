---
title: "mapValuesAsync"
description: "Returns a new object with values transformed through an async function."
---

import { Badge, Field } from '@ciderpress/ui/theme'

# mapValuesAsync <Badge color="#c85a3e">es-toolkit</Badge>
...
<Field name="getNewValue" type="(value: T[K], key: K, object: T) =&gt; Promise&lt;V&gt;" required>
An async function that generates new values. Receives value, key, and the entire object as parameters.
</Field>

<Field name="options" type="MapValuesAsyncOptions">
...
Re-exported verbatim from [`es-toolkit`](https://es-toolkit.dev/reference/object/mapValuesAsync.html). Implementation, edge cases, and performance behavior are owned upstream. This page mirrors the documentation at the pinned version; the linked source is authoritative.
