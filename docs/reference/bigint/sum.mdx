---
title: "sum"
description: "Returns the sum of all elements in an array of BigInts."
---

import { Badge, Field } from '@ciderpress/ui/theme'

# sum (for `BigInt`s) <Badge color="#c85a3e">es-toolkit</Badge>
...
The array of `BigInt`s to sum.
</Field>

#### Returns

Returns the sum of all `BigInt`s in the array. Returns `0n` for empty arrays.
...
Re-exported verbatim from [`es-toolkit`](https://es-toolkit.dev/reference/bigint/sum.html). Implementation, edge cases, and performance behavior are owned upstream. This page mirrors the documentation at the pinned version; the linked source is authoritative.
