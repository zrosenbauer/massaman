---
title: "uniqBy"
description: "Returns a new array with duplicate elements removed based on values returned by the transformation function."
---

import { Badge, Field } from '@ciderpress/ui/theme'

# uniqBy <Badge color="#c85a3e">es-toolkit</Badge>
...
<Field name="mapper" type="(item: T, index: number, array: readonly T[]) =&gt; U" required>
...
Re-exported verbatim from [`es-toolkit`](https://es-toolkit.dev/reference/array/uniqBy.html). Implementation, edge cases, and performance behavior are owned upstream. This page mirrors the documentation at the pinned version; the linked source is authoritative.
